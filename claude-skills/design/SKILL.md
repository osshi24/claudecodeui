...
source: Built-in skill của Claude Code (bundled, chức năng /design), xuất nguyên văn từ phiên bản v2.1.247. Hai file đi kèm `seed-canvas.mjs` (helper) và `payload.template.html` (editor payload ~2.4 MiB, minified — KHÔNG đọc vào context) được copy nguyên bản trong cùng thư mục này. Khi skill chạy thật, "base directory" là thư mục tạm do Claude Code giải nén (ví dụ `/private/tmp/claude-501/bundled-skills/<version>/<hash>/design`).
addendum: Phần "chạy/edit local" KHÔNG thuộc skill gốc — xem LOCAL-EDIT.md và make-local.mjs trong cùng thư mục.
---
